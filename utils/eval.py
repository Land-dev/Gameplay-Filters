...
def evaluate_single_ctrl(
  ...
  _, results, length, info = rollout_env.simulate_trajectories(
      ...
      rollout_episode_callback=rollout_episode_callback, use_tqdm=True, return_info=True
  )

  reward = sum([sum(i["reward_hist"]) for i in info]) / num_trajectories
  safe_rate = np.sum(results != -1) / num_trajectories
  info = {"safety": safe_rate, "ep_length": np.mean(length), "reward": reward}
